export const TMDB = {
  ...
  DefaultBackdrop:
    "https://images.unsplash.com/photo-1524712245354-2c4e5e7121c0?q=80&w=1632&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  DefaultCover:
    "https://images.unsplash.com/photo-1518676590629-3dcbd9c5a5c9?q=80&w=1374&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
};
import {
  getFirestore,
  doc,
...
import { similar } from "./preview.js";
// Main Url
export const MainURL = TMDB.Disc_api;
export var page = TMDB.page;
      ...
      .then((res) => {
        ...
        if (data) {
          data.forEach((el) => {
            ...
            Card.href = `./preview.html?id=${id}&page=${page}`;
            Card.id = id;
            Card.innerHTML = `
...
export function scrollBehavier() {
  ...
  Scroll_btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      } else if (carouName == "SimilarSection") {
        similar(page);
      }
    });
  });
  ...
  querySnapshot.forEach((doc) => {
    ...
    if (Animes_ids.includes(id) && Coll_name == coll_name) {
      const status = document.querySelector(
        `[data-collection-name="${coll_name}"]`,
      );
      console.log(status);
